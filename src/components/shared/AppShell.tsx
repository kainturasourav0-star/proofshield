...
export function AppShell({ role, children }: { role: "STUDENT" | "RECRUITER"; children: React.ReactNode }) {
  ...
  const scrollProgress = useSpring(scrollYProgress, { stiffness: 180, damping: 28, mass: 0.35 })
    ...
    <motion.div initial="hidden" animate="visible" className="flex h-full flex-col">
      ...
      <nav className="flex-1 space-y-1 px-3">
        {nav.map((item) => {
          ...
          return <motion.div key={item.name} whileHover={{ x: 4 }} whileTap={{ scale: .985 }} transition={{ duration: .22, ease: easing }}><Link href={item.href} onClick={() => setMobileOpen(false)} className={`group relative flex items-center gap-3 rounded-2xl px-3.5 py-3 transition-all duration-300 ${active ? "bg-lime-300/[0.09] text-lime-200" : "text-[#879481] hover:bg-white/[0.04] hover:text-[#f4f1e9]"}`}>
  ...
  return <div className="min-h-screen overflow-hidden bg-[#080b09] text-[#f4f1e9] selection:bg-lime-300/20">
    <motion.div style={{ scaleX: scrollProgress }} className="pointer-events-none fixed inset-x-0 top-0 z-[100] h-[2px] origin-left bg-lime-300" aria-hidden="true" />
